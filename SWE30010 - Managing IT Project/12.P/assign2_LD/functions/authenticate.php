<?php
	class Authentication {
		...
		public function signup($email, $profile, $pass, $date_of_birth, $address, $insurance_code) {
            ...
            $date_of_birth = stripcslashes($date_of_birth);
            $date_of_birth = mysqli_real_escape_string($connection, $date_of_birth);

            $address = stripcslashes($address);
            $address = mysqli_real_escape_string($connection, $address);

            $insurance_code = stripcslashes($insurance_code);
            $insurance_code = mysqli_real_escape_string($connection, $insurance_code);
			...
			if ($isUnique) {
				...
				$query = "INSERT INTO $table (friend_email, password, profile_name, date_started, num_of_friends, date_of_birth, friend_address, insurance_code)
                        VALUES (?,?,?,?,0,?,?,?);";
				if ($stmt = $connection->prepare($query)) {
					 $stmt->bind_param("sssssss", $email, $hashed, $profile, $date, $date_of_birth, $address, $insurance_code);
					 $stmt->execute();
					 $result = true;
				}
				else {
